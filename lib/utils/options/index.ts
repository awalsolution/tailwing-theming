...
import { MultiThemePluginOptions, ThemeConfig } from '../../theme/types'
 ...
 */
export const validateOptions = ({ defaultTheme, themes = [] }: MultiThemePluginOptions): void => {
  ...
  if (themes.some((x) => x.name === defaultThemeName)) {
    throw new Error(`No theme in the themes array in the multiThemePlugin options cannot have a name of "${defaultThemeName}"`)
  }
 ...
 * @throws an {@link Error} if the options are invalid
 */
export const getThemesFromOptions = (options: MultiThemePluginOptions): ThemeConfig[] => {
